...
// because there is not a closure until AFTER nestedFunction 
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(data){
    const littleNames = data.map(item => item.animal_name.toLowerCase());
    return littleNames;
  }
  ...
  */
  ...
  const lowAnimals = data.filter(item => item.population < 5);
  return lowAnimals
  }
  ...
  */
  ...
  function USApop(data){
    const totalPop = data.reduce((acc, num) => acc + num.population,0);
    return totalPop;
  }
...
function add(array, consumecb){
    return array + consumecb
  }


// 🦁🦁🦁 Use multiply to return the product of two numbers 🦁🦁🦁
  
function multiply(array, addcb){
   return array * addcb
  }
